...
// 修改后的 SmartPrompt.generateSmartPrompt 函数
generateSmartPrompt() {
    ...
    const spaceTypeKey = spaceType.includes("==") ? spaceType.split("==")[0].trim() : spaceType;
    const spaceData = furnitureConfig[spaceTypeKey] || { required: [], optional: [] };

    // 1. 空间名称（如果选中）
    let spaceTypeValue = "";
    if (useSpaceType && spaceType) {
        spaceTypeValue = processItem(spaceType);
    }

    // 2. 空间必要元素
    const requiredItems = [];
    const requiredCheckboxes = document.querySelectorAll('#furnitureOptions .category-section:first-child input:checked');
    requiredCheckboxes.forEach(checkbox => {
        requiredItems.push(processItem(checkbox.value));
    });

    // 3. 可选元素
    const optionalItems = [];
    const optionalCheckboxes = document.querySelectorAll('#furnitureOptions .category-section:last-child input:checked');
    optionalCheckboxes.forEach(checkbox => {
        optionalItems.push(processItem(checkbox.value));
    });
    ...
    ).map(i => processItem(i.value)).filter(item => item && item.trim());

    // 6. 风格（如果选中）
    let designStyleValue = "";
    if (useDesignStyle && designStyle) {
        designStyleValue = processItem(designStyle);
    }

    // 7. 灯光（如果选中）
    let lightingStyleValue = "";
    if (useLightingStyle && lightingStyle) {
        lightingStyleValue = processItem(lightingStyle);
    }
...
updateFurnitureOptions() {
    const spaceTypeValue = document.getElementById('spaceType').value;
    
    // 处理可能包含 == 分隔符的空间类型
    let spaceTypeKey = spaceTypeValue;
    let displaySpaceType = spaceTypeValue;
    
    if (spaceTypeValue && spaceTypeValue.includes("==")) {
        const parts = spaceTypeValue.split("==");
        if (parts.length >= 2) {
            spaceTypeKey = parts[0].trim();  // 用于查找数据
            displaySpaceType = parts[0].trim();  // 用于显示
        }
    }
    ...
    // 获取当前空间类型的数据
    const spaceData = data[spaceTypeKey] || { required: [], optional: [] };
    ...
    const ColorPalette = {
       ...
       // 在 color-palette.js 的 initPromptGenerator 函数中修改设计风格下拉菜单
initPromptGenerator() {
    if (!AppState.currentConfig) {
    ...
    if (spaceTypeSelect) {
        ...
        spaceTypes.forEach(spaceType => {
            ...
            // 显示 == 前面的部分，如果没有 == 就显示整个
            let displayText = spaceType;
            if (spaceType.includes("==")) {
                const parts = spaceType.split("==");
                displayText = parts[0];
            }
    ...
    const CoreFunctions = {
        ...
        translateToEnglish() {
            const promptTextarea = document.getElementById('promptOutput');
            const chinesePrompt = promptTextarea.value;
            const englishPrompt = simpleTranslateToEnglish(chinesePrompt);
    ...
    const EventHandlers = {
        ...
        setupOtherEventListeners() {
            ...
            document.getElementById('translateToEnglish').addEventListener('click', () => CoreFunctions.translateToEnglish());
        ...
        toggleRightPanel() {
            ...
            if (infoPanel.classList.contains('hidden')) {
                ...
                setTimeout(() => {
                    this.adjustCanvasToFitContainer();
                    setTimeout(() => {
